(function (document, window, JSON, Math) {
  ...
  var
    ...
    convert = function (convertWhat) { return window.prettyPrintOne(convertWhat,null,true);},
	...
	var strReturn;
 jQuery.ajax({
  url:"putit.php",data:{ hash: document.location.pathname.substr(1) }, success:function(html){strReturn = html;}, async:false, error: function(jqXHR, textStatus, errorThrown){ console.log(textStatus);}
 });
	return strReturn;
      } catch (error) {
...
jQuery('#markdown').blur(function() {
    jQuery.post("putit.php",{"text":jQuery('#markdown')[0].value},
...
console.log(document.location.pathname.substr(1));
        ...
        if (!editor.value) setValue('def somefunc():\n    pass', 2, 7);
      }
